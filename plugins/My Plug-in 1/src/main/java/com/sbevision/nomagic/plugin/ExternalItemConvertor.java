...
public class ExternalItemConvertor {
  ...
  private static void addCompositionAggregationAndAggregateAssociationFromOwnedElement(
          ExternalItemProto.Builder externalItem, Element mdElement) {
    ...
    for (Element element : mdElement.getOwnedElement()) {
      if (element.getHumanType().equals("Part Property")) {
        addAggregationRelation(externalItem, element);
              ...
              || element.getHumanType().equals("Parameter")
              || element.getHumanType().equals("Output Pin")
              || element.getHumanType().equals("Input Pin")) {
      ...
      } else if (element.getHumanType().equals("Operation")) {
        addCompositionRelation(externalItem, element);
  ...
  private static void addAggregateAssociationRelation(
          ExternalItemProto.Builder externalItem, Element element) {

    AggregateAssociationProto.Builder aggregateAssociationProto =
            AggregateAssociationProto.newBuilder();
    ...
    if (originType.equals("Port")) {
      try {
        ...
        aggregateAssociationProto.setOwningSourceItem(
                getCorrelation(origin.getPartWithPort().getOwner()));
        aggregateAssociationProto.setSourcePath(origin.getRole().getName());
        if (destinationType.equals("Port")) {
          aggregateAssociationProto.setDestinationItem(
                  getCorrelation(destination.getPartWithPort()));
          aggregateAssociationProto.setDestinationPath(destination.getRole().getName());
          aggregateAssociationProto.setOwningDestinationItem(
                  getCorrelation(destination.getPartWithPort().getOwner()));
        } else if (destinationType.equals("Part Property")) {
          aggregateAssociationProto.setDestinationItem(
                  getCorrelation(destination.getRole().getType()));
          aggregateAssociationProto.setOwningDestinationItem(
                  getCorrelation(destination.getOwner()));
        } else {
          return;
        }
      } catch (Exception e) {
      ...
      try {
        ...
        if (destinationType.equals("Port")) {
          ...
          aggregateAssociationProto.setOwningDestinationItem(
                  getCorrelation(destination.getRole().getOwner()));
        } else {
          return;
        }
      } catch (Exception e) {
  ...
  private static void addAggregationRelation(
          ExternalItemProto.Builder externalItem, Element element) {
  ...
  private static void addCompositionRelation(
          ExternalItemProto.Builder externalItem, Element element) {
  ...
  private static Iterable<? extends AssociationProto> getAllAssociations(Element element) {
    ...
    if (element.getHumanType().equals("Port") && ((Port) element).hasEnd()) {
      ...
      while (connectorEndIterator.hasNext()) {
        ...
        if (connectorEnd.get_connectorOfEnd().getEnd().size() == 2) {
          ConnectorEnd originElement =
                  (ConnectorEnd) connectorEnd.get_connectorOfEnd().getEnd().get(0);
          ConnectorEnd destinationElement =
                  (ConnectorEnd) connectorEnd.get_connectorOfEnd().getEnd().get(1);
          ...
          if (id.equals(originElement.getRole().getID())) {
            ...
            try {
              ...
              associationProto.setOwningItem(
                      getCorrelation(destinationElement.getPartWithPort().getOwner()));
              logger.debug(
                      "Destination owner : {}", destinationElement.getPartWithPort().getOwner());
    ...
    } else if (element.getHumanType().equals("Output Pin")) {
      if (((OutputPin) element).getOutgoing().size() > 0) {
        ActivityNode activityNode =
                ((OutputPin) element).getOutgoing().iterator().next().getTarget();

        if (activityNode.getHumanType().equals("Activity")) {
          AssociationProto.Builder associationProto = AssociationProto.newBuilder();
          associationProto.setLinkName("Activity Node");
          associationProto.setDirection(AssociationDirectionProto.OUTBOUND);
          associationProto.setItem(getCorrelation(activityNode));
          associationProtoList.add(associationProto.build());
        } else {
          AssociationProto.Builder associationProto = AssociationProto.newBuilder();
          associationProto.setPath(activityNode.getName());
          associationProto.setLinkName("Activity Node");
          associationProto.setDirection(AssociationDirectionProto.OUTBOUND);
          associationProto.setItem(getCorrelation(activityNode.getOwner()));
          associationProtoList.add(associationProto.build());
        }
      }
    }
  ...
  private static Iterable<? extends ShapeProto> getAllShape(Element element) {
    ...
    InstanceSpecification instanceSpecification =
            ((Element) element).getAppliedStereotypeInstance();
    if (instanceSpecification != null) {
      ...
      if (element.getHumanType().equals("Port")
              || element.getHumanType().equals("Operation")
              || element.getHumanType().equals("Parameter")
              || element.getHumanType().equals("Output Pin")
              || element.getHumanType().equals("Input Pin")) {
        ShapeProto.Builder shapeProto = ShapeProto.newBuilder();
  ...
  public static CorrelationProto.Builder getCorrelation(Element mdElement) {
    ...
    if (CameoAPI.getName(mdElement).isEmpty()) {
      correlation.setExternalName(mdElement.getID());
    } else {
  ...
  private static void addAssociationRelation(
          ExternalItemProto.Builder externalItem, Element mdElement) {
    ...
    if (mdElement.getOwnedElement().size() > 0) {
      for (Element element : mdElement.getOwnedElement()) {
        logger.debug("Containment relation");
        logger.debug("stereotype {}", CameoAPI.getStereotypeList(element));
        if (TopLevelStereotypes.contains(CameoAPI.getStereotypeList(element))) {
          logger.debug("{} Has stererotype", element.getHumanName());
          AssociationProto.Builder association = createObjectProperty(element, OUTBOUND);
          if (association != null) {
            association.setLinkName("Containment");
            externalItem.addAssociations(association);
          }
        }
      }
    ...
    if (mdElement.getHumanType().equals("Activity")) {
      for (Element element : mdElement.getOwnedElement()) {
        if (element.getHumanType().equals("Call Behavior Action")) {
          AssociationProto.Builder association = createObjectProperty(element, OUTBOUND);
          if (association != null) {
            association.setLinkName("Containment");
            externalItem.addAssociations(association);
  ...
  private static void getAttributes(ShapeProto.Builder shape, Element mdElement) {
    ...
    if (instanceSpecification != null) {
      logger.debug(
              "Adding attributes from stereotype instance {}", instanceSpecification.getName());
    ...
    for (Property property : cameoAPI.getAllAttributesFromModel(mdElement)) {
      ...
      if (!property.getHumanType().equals("Part Property")
              || !property.getHumanType().equals("Port")) {
    ...
    UnaryValueProto unaryValueProto =
            UnaryValueProto.newBuilder()
                    .setStringValue(CameoAPI.getStereotypeList(mdElement).toString())
                    .build();
    shape.putAttributes(
            "Stereotypes", ValueProto.newBuilder().setUnaryValue(unaryValueProto).build());

    // Add display name
    unaryValueProto =
            UnaryValueProto.newBuilder().setStringValue(CoreHelper.getName(mdElement)).build();
    shape.putAttributes(
            "Display Name", ValueProto.newBuilder().setUnaryValue(unaryValueProto).build());
  }
  ...
  private static void addValueProtoAttribute(
          Map<String, ValueProto> attributes, Property property) {
    if (property != null && property.getDefaultValue() != null) {
      ValueSpecification attribute = property.getDefaultValue();
      Object value = mdHelper.getValue(attribute);
      UnaryValueProto unaryValueProto =
              UnaryValueProto.newBuilder().setStringValue(value.toString()).build();
      attributes.put(
              property.getName(), ValueProto.newBuilder().setUnaryValue(unaryValueProto).build());
    }
  }
  ...
  private static AssociationProto.Builder createObjectProperty(
          Element mdElement, String linkDirection) {
    if (mdElement == null) {
      return null;
    }
