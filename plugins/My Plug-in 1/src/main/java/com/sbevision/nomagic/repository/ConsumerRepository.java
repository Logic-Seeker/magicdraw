...
//    Metadata.Key<String> key = Metadata.Key.of(Environment.AUTHORIZATION, Metadata.ASCII_STRING_MARSHALLER);
...
//
//        originChannel = NettyChannelBuilder.forAddress(Environment.SBE_CONNECTOR_HOST, Environment.SBE_CONNECTOR_PORT).sslContext(sslcontext).intercept(MetadataUtils.newAttachHeadersInterceptor(getHeader())).build();
//      } else {
//        originChannel = NettyChannelBuilder.forAddress(Environment.SBE_CONNECTOR_HOST, Environment.SBE_CONNECTOR_PORT).intercept(MetadataUtils.newAttachHeadersInterceptor(getHeader())).usePlaintext().build();
//      }
//    } catch (Exception e) {
...
//    if (channel == null || ConnectivityState.READY != channel.getState(true) || blockingStub == null) {
...
//
//    SubscriptionThread subscriptionThread = new SubscriptionThread(pullRequestProto, getAsyncStub());
